...
async function updateTeamPage() {
  ...
  setText("envHum", isNum(toNum(m?.humidityPct)) ? `${fmt(toNum(m.humidityPct), 0)}%` : "—");
...
function updateRiskPage() {
  ...
  const ffNames = {
    FF_A: "Firefighter A (Alex)",
    FF_B: "Firefighter B (Maria)",
    FF_C: "Firefighter C (Nikos)",
    FF_D: "Firefighter D (Eleni)"
  };
    ...
    tbody.innerHTML = rows
      .map((r) => {
        ...
        <tr class="risk-row" data-ff="${escapeHtml(r.ffId)}"${active}>
          <td><b>${escapeHtml(ffNames[r.ffId] || r.ffId)}</b></td>
...
function updateMedicalSidePanels() {
  ...
  if (mm) {
    mm.innerHTML = [
      card("WS (Web Socket)", ECG.connected ? "LIVE" : "OFF"),
      card("ECG fs (Sampling Rate)", isNum(ECG.fs) ? `${fmt(ECG.fs, 1)} Hz` : "—"),
  ...
  if (pm) {
    pm.innerHTML = [
      card("HR (Heart Rate)", isNum(m.hrBpm) ? `${fmt(m.hrBpm, 0)} bpm` : "—"),
      card("RR (Respiratory Rate)", isNum(m.rrMs) ? `${fmt(m.rrMs, 0)} ms` : "—"),
      card("RMSSD (Recovery Strength)", isNum(m.rmssdMs) ? `${fmt(m.rmssdMs, 1)} ms` : "—"),
      card("SDNN (Cardiac Flexibility)", isNum(m.sdnnMs) ? `${fmt(m.sdnnMs, 1)} ms` : "—"),
      card("pNN50 (Recovery Events)", isNum(m.pnn50Pct) ? `${fmt(m.pnn50Pct, 1)} %` : "—"),
...
async function poll() {
  ...
  try {
    ...
    // MAP TAB humidity: Weather:Patra has no humidity, so show leader's local humidity (EnvNode)
    try {
      const leader = state.latestByFf["FF_A"] || {};
      const hum = toNum(leader.humidityPct);
      const wxHumEl = document.getElementById("wxHum");
      if (wxHumEl) wxHumEl.textContent = isNum(hum) ? `${fmt(hum, 0)}%` : "—";
    } catch (_) {}
