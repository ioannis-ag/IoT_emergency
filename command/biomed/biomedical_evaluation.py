...
def parse_ecg1_simple(payload: bytes):
    """
    Simple ESP32 format:
    'ECG1'(4) + ms(4) + n(uint16) + hr(uint16) + stress(uint8) + n*int16 samples
    Returns (ms, hr, stress_0_1, samples_int16) or (None,... ) if not matching.
    """
    if len(payload) < 13 or payload[0:4] != b"ECG1":
        return None, None, None, None

    ms = int.from_bytes(payload[4:8], "little", signed=False)
    n  = int.from_bytes(payload[8:10], "little", signed=False)
    hr = int.from_bytes(payload[10:12], "little", signed=False)
    st = payload[12] / 255.0

    need = 13 + 2 * n
    if n == 0 or len(payload) < need:
        return None, None, None, None

    # int16 little-endian
    samples = np.frombuffer(payload[13:13 + 2*n], dtype="<i2").astype(np.int32)
    return ms, hr, st, samples
...
def on_message(client, userdata, msg):
    ...
    # ---------------- Optional hints ingestion ----------------
    if msg.topic.startswith("ngsi/Biomedical/") or msg.topic.startswith("ngsi/Environment/"):
        try:
            j = json.loads(msg.payload.decode("utf-8", errors="ignore"))
            if "motionLevel" in j:
                st.motion_level = float(j["motionLevel"])
            if "activity" in j:
                st.activity = str(j["activity"])
        except Exception:
            pass
        return

    # ---------------- ECG ingestion ----------------
    # Your project has TWO possible "ECG1" payload formats:
    # (A) "simple" ESP32 replay: ECG1 + ms + n + hr + stress + n*int16
    # (B) "bundle" format: ECG1 + ... + count + [lenLE + PMD_pkt]...
    #
    # We'll try to parse (A) first; if it fails, fall back to (B).
    ...
    def parse_ecg1_simple(payload: bytes):
        ...
        n  = int.from_bytes(payload[8:10], "little", signed=False)
        hr = int.from_bytes(payload[10:12], "little", signed=False)
        stf = payload[12] / 255.0
        need = 13 + 2 * n
        if n == 0 or len(payload) < need:
            return None, None, None, None
        samples = np.frombuffer(payload[13:13 + 2*n], dtype="<i2").astype(np.int32)
        return ms, hr, stf, samples

    ms, hr_hint, stress_hint, simple_samples = parse_ecg1_simple(msg.payload)

    if simple_samples is not None:
        # ---- SIMPLE ESP32 ECG ----
        # No true timestamp per sample -> fs stays DEFAULT_FS unless you change it elsewhere.
        # NOTE: if FF_B is still sending 20 samples every 50ms, that's ~400 Hz and DEFAULT_FS=130 will be wrong.
        for v in simple_samples:
            st.buf.append(int(v))
    ...
    t_now = time.time()
    if (t_now - st.last_pub_t) < PUBLISH_EVERY_SEC:
        return

    nwin = min(int(WINDOW_SEC * fs), len(st.buf))
    raw = np.array(list(st.buf)[-nwin:], dtype=np.float64)
    ...
    # baseline update (good SQI only)
    update_baselines(st, bpm, rmssd, sqi)

    tc = core_temp_estimate_c(bpm, st.baseline_hr, ambient_c=None)
    psi = psi_index(bpm, tc)

    fatigue = fatigue_score_0_100(
        bpm=bpm,
        rmssd=rmssd,
        lf_hf=fd.get("lf_hf"),
        stress_index=si,
        sqi=sqi,
        baseline_hr=st.baseline_hr,
        baseline_rmssd=st.baseline_rmssd,
        motion_level=st.motion_level
    )

    cardiac = cardiac_concern_0_100(
        bpm=bpm,
        cvrr=cvrr,
        ectopy_ratio=ect,
        sqi=sqi
    ...
    payload = {
        ...
        **fd,
        ...
        "baseline_ready": bool(st.baseline_ready),

        # hints
        "motionLevel": st.motion_level,
        "activity": st.activity,

        # derived indices
        "tc_est_c": tc,
        "psi": psi,
        "fatigue_score_0_100": fatigue,
        "cardiac_concern_0_100": cardiac,
    }
    ...
    if sqi < LOW_SQI:
        ...
        if psi is not None:
            if psi >= PSI_DANGER:
                alerts.append({"type": "heat_strain", "severity": "danger", "msg": f"High strain PSI {psi:.1f} (Tc {tc:.2f}°C)"})
            elif psi >= PSI_WARN:
                alerts.append({"type": "heat_strain", "severity": "warn", "msg": f"Rising strain PSI {psi:.1f} (Tc {tc:.2f}°C)"})
        if fatigue is not None and fatigue >= 75:
            alerts.append({"type": "fatigue", "severity": "warn", "msg": f"High fatigue/load {fatigue:.0f}/100"})
        if cardiac is not None and cardiac >= 75:
            alerts.append({"type": "cardiac_concern", "severity": "danger", "msg": f"Elevated cardiac concern {cardiac:.0f}/100"})

    publish(client, team, ff, payload, alerts)
    st.last_pub_t = t_now
