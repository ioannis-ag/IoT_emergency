...
INCIDENT_TOPIC_PREFIX = "edge/incident"  # publishes to edge/incident/Team_A
...
# MORE FREQUENT UPDATES -> smoother, smaller movement
FAKE_LOC_HZ = 2.0

# Human movement limits (smaller steps)
MAX_SPEED_MPS = 0.85
MAX_ACCEL_MPS2 = 0.18
...
# =========================================================
# Incident model (smaller + closer to leader)
# =========================================================
INCIDENT_RADIUS_M = 22.0
INCIDENT_LEAD_START_M = 70.0
INCIDENT_LEAD_MIN_M = 25.0
INCIDENT_LEAD_SHRINK_MPS = 0.12
INCIDENT_MIN_MOVE_M = 1.2

# ✅ NEW: incident center updates only every 10 minutes
INCIDENT_UPDATE_INTERVAL_SEC = 600.0  # 10 min

# Staging offsets relative to incident (tighter)
STAGE_OFFSET_M = 8.0
C_PUSH_IN_M = 6.0
...
incident = {
    ...
    "last_update_ts": 0.0,  # ✅ NEW: last time we recomputed center
}
...
# INCIDENT MODEL
# =========================================================
def estimate_heading_unit() -> Tuple[float, float]:
    """
    Estimate A direction as unit vector in local meters (east, north).
    If insufficient/too small motion, return stable default.
    """
    tnow = time.time()
    while _a_hist and (tnow - _a_hist[0][0]) > 20.0:
        _a_hist.pop(0)
...
def update_incident_from_a(a_lat: float, a_lon: float, dt: float):
    ...
    dt here should be the elapsed time since last incident recompute.
    """
    # shrink lead over time (so A "approaches" realistically)
    lead = float(incident["leadM"])
    lead = max(INCIDENT_LEAD_MIN_M, lead - INCIDENT_LEAD_SHRINK_MPS * dt)
    incident["leadM"] = lead

    ux, uy = estimate_heading_unit()
...
def build_ui_summary(team: str, ff: str):
    ...
    if isinstance(dist_inc, (int, float)):
        ...
        if dist_inc <= r:
            severity = "danger"
        elif dist_inc <= (r + 18) and _severity_rank(severity) < _severity_rank("warn"):
            severity = "warn"
    ...
    if isinstance(dist_inc, (int, float)):
        ...
        elif _persist_ok("inc_near", dist_inc <= (r + 18)):
...
def publish_ui(client: mqtt.Client, team: str, ff: str):
    ...
    if sig != _last_alert_sig.get(key) and can_publish:
...
# =========================================================
# FAKE LOCATION SIM (goal-seeking, smooth, tiny steps)
# =========================================================
class GoalWalker:
    ...
    def step_to_goal(self, dt: float, goal_lat: float, goal_lon: float):
        ...
        if dist < 6.0:
            desired_speed = MAX_SPEED_MPS * (dist / 6.0)

        desired_vx = ux * desired_speed
        desired_vy = uy * desired_speed

        ax = (desired_vx - self.vx) * 0.85
        ay = (desired_vy - self.vy) * 0.85

        self._noise_phase += 0.18 * dt
...
def incident_targets(a_lat: float, a_lon: float) -> Dict[str, Tuple[float, float]]:
    ...
    b_x = base_out_x + px * 8.0
    b_y = base_out_y + py * 8.0

    d_x = base_out_x - px * 7.0
    d_y = base_out_y - py * 7.0

    t = time.time()
    pulse = 0.5 + 0.5 * math.sin(t * 0.18)
    desired_r = (r + 3.0) - pulse * (C_PUSH_IN_M + 3.0)  # ~r+3 down to ~r-6
    c_x = ux * desired_r + px * 3.0
    c_y = uy * desired_r + py * 3.0
...
def fake_loop(client):
    ...
    al = float(last_loc[(TEAM_ID, REAL_FF_ID)].get("lat", FALLBACK_LAT))
    ao = float(last_loc[(TEAM_ID, REAL_FF_ID)].get("lon", FALLBACK_LON))
    logging.info("Fake sim: anchor acquired (%.6f, %.6f)", al, ao)

    # Initialize incident ONCE immediately
    update_incident_from_a(al, ao, dt=1.0)
    incident["last_update_ts"] = time.time()
    publish_incident(client)

    # Start near A
    walker_B.attach_near(al, ao, 18.0)
    walker_C.attach_near(al, ao, 22.0)
    walker_D.attach_near(al, ao, 16.0)
    ...
    while True:
        t = time.time()
        dt = max(0.05, min(0.8, t - last_t))
        last_t = t

        # Refresh A anchor from latest state (do NOT append history here; on_message handles it)
        ...
        # ✅ Update incident ONLY every 10 minutes (and shrink lead based on real elapsed time)
        now_ts = time.time()
        if (now_ts - incident["last_update_ts"]) >= INCIDENT_UPDATE_INTERVAL_SEC:
            dt_inc = now_ts - incident["last_update_ts"]
            dt_inc = max(1.0, min(dt_inc, INCIDENT_UPDATE_INTERVAL_SEC * 2.0))  # clamp
            update_incident_from_a(al, ao, dt=dt_inc)
            incident["last_update_ts"] = now_ts
            publish_incident(client)

        # Move B/C/D around stable incident
...
def on_message(client, userdata, msg):
    ...
    if etype == "Location" and team and ff:
        ...
        if isinstance(payload.get("lat"), (int, float)) and isinstance(payload.get("lon"), (int, float)):
            ...
            # Track A movement for heading estimate
            if ff == REAL_FF_ID and team == TEAM_ID:
                _a_hist.append((time.time(), float(payload.get("lat")), float(payload.get("lon"))))

                # If incident not initialized yet, initialize immediately once
                if incident["lat"] is None or incident["lon"] is None:
                    update_incident_from_a(float(payload.get("lat")), float(payload.get("lon")), dt=1.0)
                    incident["last_update_ts"] = time.time()
                    publish_incident(client)
