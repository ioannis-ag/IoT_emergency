...
ORION_ALLOWED_ATTRS = {
    # common IDs
    "teamId", "ffId", "nodeId", "originNodeId", "via", "source",

    # location
    "lat", "lon", "accuracyM", "activity",

    # env
    "tempC", "humidityPct", "mq2Raw", "mq2Digital", "coPpm",

    # biomed
    "hrBpm", "rrMs", "wearableOk",

    # misc
    "rssi", "battery", "failover",
...
def filter_orion_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for k, v in payload.items():
        ...
        if isinstance(v, (list, dict)):
            # lists/dicts are not allowed (bridge skips them too)
            continue
...
_a_hist: List[Tuple[float, float, float]] = []  # [(t, lat, lon)]
...
def estimate_heading_unit() -> Tuple[float, float]:
    ...
    if len(_a_hist) < 2:
        return (0.7, 0.7)
    _, lat0, lon0 = _a_hist[0]
    _, lat1, lon1 = _a_hist[-1]
...
def incident_targets(a_lat: float, a_lon: float) -> Dict[str, Tuple[float, float]]:
    ...
    desired_r = (r + 3.0) - pulse * (C_PUSH_IN_M + 3.0)
...
def fake_loop(client):
    ...
    while True:
        ...
        if (now_ts - incident["last_update_ts"]) >= INCIDENT_UPDATE_INTERVAL_SEC:
            ...
            dt_inc = max(1.0, min(dt_inc, INCIDENT_UPDATE_INTERVAL_SEC * 2.0))
...
def on_message(client, userdata, msg):
    # We do NOT forward raw ECG to Orion here (it’s too heavy + not normalized)
    ...
    # ✅ IMPORTANT: normalize Location messages so they ALWAYS have teamId/ffId
    if etype == "Location" and team and ff:
        payload.setdefault("teamId", team)
        payload.setdefault("ffId", ff)

        # OwnTracks: acc -> accuracyM
        if "accuracyM" not in payload and "acc" in payload:
            payload["accuracyM"] = payload.get("acc")

        # OwnTracks: batt -> battery (optional)
        if "battery" not in payload and "batt" in payload:
            payload["battery"] = payload.get("batt")

        # OwnTracks: motionactivities is a LIST -> extract first string into "activity"
        if "activity" not in payload and isinstance(payload.get("motionactivities"), list) and payload["motionactivities"]:
            first = payload["motionactivities"][0]
            if isinstance(first, str):
                payload["activity"] = first

    filtered = filter_orion_payload(payload)
    if filtered:
        send_to_orion(build_ngsi_entity(eid, etype, filtered))

    # Update local state for UI publishing
    if etype == "Location" and team and ff:
